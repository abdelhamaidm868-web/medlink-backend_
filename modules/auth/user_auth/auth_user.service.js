  ...
  try {
    ...
    db.execute(checkQuery, [email], async (err, results) => {
      try {
        ...
        // تم إصلاح خطأ هنا: عدد الأعمدة يجب أن يساوي عدد القيم الممررة
        const insertQuery =
          "INSERT INTO users (name, email , location , phone, password ) VALUES (? , ? , ? , ? , ? )";

        db.execute(
          insertQuery,
          [name, email, location, phone, hashedPassword], // تم إزالة role لأنه غير معُرف في الاستعلام
          (err, result) => {
            if (err) {
              console.log(err);
...
export const userLogin = (req, res) => {
  try {
    ...
    db.query(checkQuery, [email], async (err, result) => {
      ...
      // تم حل التعارض هنا واختيار الكود الصحيح
